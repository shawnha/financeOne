...
1. ✅ **선수금/25300 — 결산 권위로 해소(2026-06-07)**: 선수금 canonical=**25900**(홀세일 결산; 코리아엔 선수금 없음). **25300=미지급금**(결산이 25300을 미지급금으로 사용 — 카드대금 클리어링+현대캐피탈 차량금융; DB 선수금 라벨 오류→개명). 25300 booking 8건은 전부 미지급금이 맞음(재분류 불필요). 미지급금 canonical=**25300**(결산), DB 25200→25300 통합. 코리아만 미지급금 없음→카드대금=26200. 상세 context-notes. (단 "자동차할부/대출이자" 명목의 **이자 성격** 라인은 이자비용 93100으로 — 병행 분류작업 영역.)
2. **gaap_mapping 신규 행** — 사용>0·gaap=0인 한국 결산코드 ~12개(82200·29300·26000·93100 등)의 US GAAP 대응 코드 = 결산/QBO 권위 필요. (전역 일원화 마이그 시점, PoC 분리됨.)
3. **canonical 42그룹 is_relabel 판정** — 그룹별 old/new의 (category, subcategory, normal_side) 동일성 실측 게이트 결과에 따라 재라벨 vs 재분류 분기(자동 가정 금지). (전역 일원화 마이그.)
4. ✅ **마감 기준일 = 2025-12까지 잠금**(2026~ 열림). 사장님 결정 2026-06-07. 25년귀속 결산 신고완료 근거. fiscal_period_locks에 전 법인 2025-12 이하 일괄 등록.
5. ✅ **홀세일 GL 정책 = (b) 현행 유지**. 사장님 결정 2026-06-07. 분개 안 만듦 — 홀세일 재무제표=transactions+wholesale_* 발생주의 명시, 연결 special-case. 계정 정리는 거래 매핑/명칭만(GL 백필 안 함). drift 정렬도 transactions 레벨(잠긴 2025 이하는 재분류, 2026은 전향).
