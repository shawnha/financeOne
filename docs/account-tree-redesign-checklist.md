...
## Phase 2.5 — 구현 전 잠금 (결산권위 입력 + 설계 개정)
- [x] 결산 권위 cross-check(2026-06-07): 선수금=**25900**·25300=**미지급금**(현대캐피탈 차량금융 포함)·미지급금=25300(DB 25200 통합)·코리아 카드대금=26200. 결산 원장 1차 사료 확증.
- [x] 사장님 결정: 마감 기준일=**2025-12까지 잠금**(2026~ 열림)·홀세일 GL=**현행 유지**(분개 안 만듦).
- [ ] gaap 신규 12코드(US 대응) — 전역 일원화 마이그 시점(PoC 분리됨, 후순위)
- [ ] §14 반영해 §1/§3/§7 개정: GL재전기=std 라인만 in-place(C1)·잠금 가드 전 삭제경로+DB트리거(C2/C3)·잠금 입력테이블 확장(C4)·diff in-memory(H3)
- [ ] (필요시) 개정 설계 재-리뷰

## 병행 트랙 (다른 세션)
- 4법인 트리 목업(`_gen_tree_all.py`)·내부잎 법인별 분리(시드오염 정리)·5월 손익 리포트 = 별도 세션 진행 중. 분류/시각화 레이어. 본 트랙(마이그 메커니즘)과 결론 일치(25300=미지급금 등).
